...
public class HttpCommon {
	...
	public JSONObject getJsonResponseWithJSONmessage(String url,
		...
		try {
			...
			if (statusLine.getStatusCode() == 200) {
				String responseString = EntityUtils.toString(httpResponse
						.getEntity());
				JSONObject rejson = new JSONObject(responseString);
				return rejson;
			} else {
				throw new Exception("access server fail");
			}
	...
	public JSONObject getJsonResponseWithParams(String url,
			Map<String, String> params, Map<String, String> files) {
		try {
			URI uri = new URIBuilder().setPath(url).build();
			HttpPost httpPost = new HttpPost(uri);
			MultipartEntityBuilder   multiEntity = MultipartEntityBuilder.create();
			multiEntity.setMode(HttpMultipartMode.BROWSER_COMPATIBLE);
			if (params != null) {
//				List<NameValuePair> nvps = new ArrayList<NameValuePair>();
				for (String key : params.keySet()) {
//					nvps.add(new BasicNameValuePair(key, params.get(key)));
					multiEntity.addTextBody(key,params.get(key));
				}
//				httpPost.setEntity(new UrlEncodedFormEntity(nvps));
			}
			for(String key: files.keySet()){
				FileBody filebody = new FileBody(new File(files.get(key)));
	...
	public boolean downloadFile(String url, Map<String, String> params,
			String saveFilePath) {
		try {
			URI uri = new URIBuilder().setPath(url).build();
			HttpPost httpPost = new HttpPost(uri);
			...
			if (statusLine.getStatusCode() == 200) {
				...
				SaveFileUtil.saveFile(saveFilePath, inputStream);
